# Sovereign Grid Self-Initiation Bootstrap v1.0.0 + Vault & Archive v1.0.2
...
### Vault & Archive Module (v1.0.2)
- **`sovereign_grid_vault_archive.py`** — Gate 158 crypto pipeline router, HMAC-sealed vault, append-only hash-chained archive, Stellar/BENJI pipeline binding, full TSU3.12 TSA certificate (RSA modulus + policy OID).
...
```bash
python3 sovereign_grid_vault_archive.py   # v1.0.2 — full TSU3.12 RSA modulus + policy + issuer OU
```
...
- Stores: crypto identifier (secp256k1 / BTC address / WIF), Stellar BENJI pipeline, full TSA certificate (TSU3.12)
